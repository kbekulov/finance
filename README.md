...
Treat every theme as one shared interface with a distinct palette, not as a separate layout. Each theme must define accessible focus, timeline, chart, divider, semantic positive, and semantic negative colors; keep red and green meanings consistent even when decorative accents differ. QA all themes at desktop and a 375-pixel mobile viewport. Small text and controls must remain legible, hover states must preserve the theme's surface contrast, important card content must not clip, and theme-specific styling must not introduce glow effects.
