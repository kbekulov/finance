...
Every expense row uses a category-specific transparent Fate/stay night chibi PNG from `public/category-icons/` instead of a letter monogram. Keep the category-to-icon map complete and identical in JavaScript and React whenever categories change. Treat the image as decorative because the localized category name remains visible in text, preserve the original square aspect ratio, and use a borderless cutout with a restrained shadow rather than another badge or card. Design for the actual 48-pixel UI size. Keep the whole set in one shared waist-up template with consistent head scale, bold dark contours, crisp cel shading, a limited high-contrast palette, and one oversized category metaphor that occupies roughly two fifths of the canvas. Judge every replacement on both light and dark backgrounds at 48 pixels. Avoid tiny secondary props or symbolism that only reads at full resolution, and create new concepts from a blank canvas instead of tracing an older icon. The current cast and cues are Taiga Fujimura as a curry-shop cook hugging a giant curry platter for Food, Caster Medea as a digital archivist with a recurring-services calendar wheel for Subscriptions & services, Luviagelita Edelfelt as a couture buyer presenting a sapphire clutch for Luxury purchases, Kirei Kotomine as a debt-release officer breaking a shackle around a coin for Debt & repayments, Archer EMIYA as an electronics mechanic holding an opened circuit-heart game console for Devices & installments, Rider Medusa as a subway conductor presenting a train nose for Transport & Travel, and Lancer Cu Chulainn as a pub tapmaster hoisting a ruby beer stein for Alcohol & nightlife.
