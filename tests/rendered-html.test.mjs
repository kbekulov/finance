...
test("server-renders the current finance tracker", async () => {
  ...
  assert.match(html, /Самая крупная статья расходов: Еда, 583,18/);
  ...
  assert.match(html, /1.{0,4}077,92.{0,8}€/);
  ...
  assert.match(html, /2.{0,4}014,39.{0,8}€/);
  assert.match(html, /145,61.{0,8}€/);
  assert.match(html, /-54,39.{0,8}€/);
  ...
  assert.match(html, /Учтено расходов: 55/);
  ...
  const renderedAllFundsPace = Math.round(145.61 / renderedRemainingDays);
...
test("keeps database history and translations aligned", async () => {
  ...
  assert.equal(current.revision, 38);
  assert.equal(current.savingsGoal, 200);
  assert.equal(current.expenses.length, 55);
  ...
  assert.equal(spentCents, 201439);
  assert.equal(recurringCents, 93647);
  assert.equal(oneTimeCents, 107792);
  ...
  assert.equal(cashRemainingCents, 14561);
  assert.equal(safeRemainingCents, -5439);
  ...
  assert.equal(Math.round((cashRemainingCents / 100) / remainingDaysAfterToday), 10);
  ...
  assert.deepEqual(salaryHistoryScenario.map(({ cashRemaining }) => cashRemaining), [145.61, 735.61]);
  assert.deepEqual(salaryHistoryScenario.map(({ safeRemaining }) => safeRemaining), [-54.39, 535.61]);
