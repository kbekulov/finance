...
test("server-renders the current finance tracker", async () => {
  ...
  assert.match(html, /Самая крупная статья расходов: Еда, 596,07/);
  assert.match(html, /Устройства Apple/);
  assert.match(html, /iPad/);
  assert.match(html, /Обновлено 28 июля 2026 г\./);
  ...
  assert.match(html, /1.{0,4}090,81.{0,8}€/);
  ...
  assert.match(html, /2.{0,4}027,28.{0,8}€/);
  assert.match(html, /132,72.{0,8}€/);
  assert.match(html, /-67,28.{0,8}€/);
  ...
  assert.match(html, /Учтено расходов: 58/);
  assert.match(html, /94%<!-- --> <!-- -->ПОТРАЧЕНО|94% ПОТРАЧЕНО/);
  ...
  const renderedAllFundsPace = Math.round(132.72 / renderedRemainingDays);
  ...
  assert.match(html, /allowance-guide-all" style="--guide-top:84\.72%"/);
...
test("keeps database history and translations aligned", async () => {
  const [databaseText, strengthText, bannerManifestText, page, script, styles, index, manual] = await Promise.all([
    ...
    readFile(new URL("../public/theme-banners/banner-manifest.json", import.meta.url), "utf8"),
  ...
  const bannerManifest = JSON.parse(bannerManifestText);
  ...
  assert.deepEqual(bannerManifest, {
    timezone: "Europe/Vilnius",
    lastGeneratedOn: "2026-07-28",
    topic: "Two everyday food purchases",
    queryVersion: 12,
    frames: ["kinance.png", "kinance-frame-2.png"],
  });
  assert.equal(current.updatedAt, "2026-07-28");
  assert.equal(current.revision, 39);
  assert.equal(current.savingsGoal, 200);
  assert.equal(current.expenses.length, 58);
  assert.deepEqual(current.expenses.slice(0, 5), [
    {
      id: "2026-07-balance-reconciliation-groceries-001",
      amount: 6.4,
      note: "Groceries",
      noteTranslations: { en: "Groceries", ru: "Продукты" },
      date: "2026-07-28",
      category: "Food",
      source: "chat",
      paymentMethod: "debit",
      synthetic: true,
      backfillBatch: "2026-07-debit-balance-reconciliation-2026-07-28",
    },
    {
      id: "2026-07-balance-reconciliation-snack-001",
      amount: 3.2,
      note: "Snack",
      noteTranslations: { en: "Snack", ru: "Перекус" },
      date: "2026-07-28",
      category: "Food",
      source: "chat",
      paymentMethod: "debit",
    ...
    {
      id: "2026-07-balance-reconciliation-soft-drink-001",
      amount: 3.29,
      note: "Soft drink",
      noteTranslations: { en: "Soft drink", ru: "Безалкогольный напиток" },
      date: "2026-07-28",
      category: "Food",
      source: "chat",
      paymentMethod: "debit",
    ...
    },
    {
      ...
      amount: 7,
      note: "Food purchase",
      noteTranslations: { en: "Food purchase", ru: "Покупка еды" },
      date: "2026-07-28",
      category: "Food",
      source: "chat",
      paymentMethod: "debit",
  ...
  const balanceReconciliation = current.expenses.filter(
    (expense) =>
      expense.backfillBatch === "2026-07-debit-balance-reconciliation-2026-07-28",
  );
  assert.equal(balanceReconciliation.length, 3);
  assert.equal(sumCents(balanceReconciliation), 1289);
  for (const expense of balanceReconciliation) {
    assert.equal(expense.synthetic, true);
    assert.equal(expense.source, "chat");
    assert.equal(expense.paymentMethod, "debit");
    assert.equal(expense.date, "2026-07-28");
    assert.equal(expense.category, "Food");
    assert.equal(expense.recurring, undefined);
  }
  assert.equal(spentCents, 202728);
  assert.equal(recurringCents, 93647);
  assert.equal(oneTimeCents, 109081);
  ...
  assert.equal(cashRemainingCents, 13272);
  assert.equal(safeRemainingCents, -6728);
  ...
  const elapsedCycleDays = calendarDay("2026-07-28") - calendarDay(current.period.start) + 1;
  const remainingDaysAfterToday = totalCycleDays - elapsedCycleDays;
  assert.equal(totalCycleDays, 33);
  assert.equal(elapsedCycleDays, 19);
  assert.equal(remainingDaysAfterToday, 14);
  ...
  assert.equal(Math.round((spentCents / totalIncomeCents) * 100), 94);
  ...
  assert.equal(todaySpentEuros, 33.05);
  ...
  const salaryHistoryScenario = [
    { salary: 2150, additionalIncome: 10, savingsGoal: 200, spent: 2027.28 },
    { salary: 2750, additionalIncome: 0, savingsGoal: 200, spent: 2027.28 },
  ].map((cycle) => ({
  ...
  assert.deepEqual(salaryHistoryScenario.map(({ cashRemaining }) => cashRemaining), [132.72, 722.72]);
  assert.deepEqual(salaryHistoryScenario.map(({ safeRemaining }) => safeRemaining), [-67.28, 522.72]);
  ...
  assert.match(index, /public\/theme-banners\/kinance\.png\?v=12/);
  assert.equal((script.match(/public\/theme-banners\/kinance\.png\?v=12/g) ?? []).length, 2);
  assert.equal((page.match(/theme-banners\/kinance\.png\?v=12/g) ?? []).length, 2);
  assert.equal((script.match(/public\/theme-banners\/kinance-frame-2\.png\?v=12/g) ?? []).length, 2);
  assert.equal((page.match(/theme-banners\/kinance-frame-2\.png\?v=12/g) ?? []).length, 2);
