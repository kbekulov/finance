...
test("server-renders the current finance tracker", async () => {
  ...
  assert.match(html, /1.{0,4}110,57.{0,8}€/);
  ...
  assert.match(html, /2.{0,4}047,04.{0,8}€/);
  assert.match(html, /112,96.{0,8}€/);
  assert.match(html, /-87,04.{0,8}€/);
  ...
  assert.match(html, /Учтено расходов: 61/);
...
test("keeps database history and translations aligned", async () => {
  ...
  assert.equal(current.expenses.length, 61);
  assert.deepEqual(current.expenses.slice(0, 5), [
    {
      id: "2026-07-gift-001",
      amount: 13.98,
      note: "Gift",
      noteTranslations: { en: "Gift", ru: "Подарок" },
      date: "2026-07-28",
      category: "Luxury purchases",
      source: "chat",
      paymentMethod: "debit",
    },
    {
  ...
  assert.equal(spentCents, 204704);
  assert.equal(recurringCents, 93647);
  assert.equal(oneTimeCents, 111057);
  ...
  assert.equal(cashRemainingCents, 11296);
  assert.equal(safeRemainingCents, -8704);
  ...
  assert.equal(todaySpentEuros, 52.81);
  ...
  const salaryHistoryScenario = [
    { salary: 2150, additionalIncome: 10, savingsGoal: 200, spent: 2047.04 },
    { salary: 2750, additionalIncome: 0, savingsGoal: 200, spent: 2047.04 },
  ].map((cycle) => ({
  ...
  assert.deepEqual(salaryHistoryScenario.map(({ cashRemaining }) => cashRemaining), [112.96, 702.96]);
  assert.deepEqual(salaryHistoryScenario.map(({ safeRemaining }) => safeRemaining), [-87.04, 502.96]);
